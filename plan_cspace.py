...
def configCollidesWithBoxes(q, base, segments, boxes):
    s = [(q[i], seg[1]) for i, seg in enumerate(segments)]
    pts = mat.forward(base, s)
    p0 = np.array([[pts[0] - base]])
    p1 = np.array([[pts[1] - base]])
    p2 = np.array([[pts[2] - base]])
...
while True:
    ...
    if key == ord('p'):
        ...
        planner = RRT(pstart, ptarget, pshape, 0.1, 0.01, 0.1, lambda q: configCollidesWithBoxes(q, base, segments, boxes))
